...
PUBLIC FUNCTION list(l_token STRING ATTRIBUTE(WSParam))
		ATTRIBUTES(WSPath = "/list/{l_token}", WSGet, WSDescription = "Get a list of Customers")
...
#+ result: An array of customers
PUBLIC FUNCTION get(l_key STRING ATTRIBUTE(WSParam), l_token STRING ATTRIBUTE(WSParam))
		ATTRIBUTES(WSPath = "/get/{l_key}/{l_token}", WSGet, WSDescription = "Get a Customers")
		RETURNS(t_customers ATTRIBUTES(WSMedia = 'application/json'))
	CALL m_customers.arr.clear()
	IF ws_lib.checkToken(l_token) THEN
