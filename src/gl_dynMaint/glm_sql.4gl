...
FUNCTION glm_mkSQL(l_cols STRING, l_where STRING)
	...
	CATCH
		CALL g2_lib.g2_errPopup(SFMT(%"Failed to doing prepare for select from '%1'\n%2!", m_tab, SQLERRMESSAGE))
		EXIT PROGRAM
	END TRY
	...
	END FOR
	IF m_key_fld = 0 THEN
		CALL g2_lib.g2_errPopup(SFMT(%"The key field '%1' doesn't appear to be in the table!", m_key_nam.trim()))
		EXIT PROGRAM
	END IF
	IF l_where != "1=2" THEN
