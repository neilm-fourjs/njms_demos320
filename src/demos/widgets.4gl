			...
			ON ACTION gl_lookup
				...
				MENU "" ATTRIBUTES(STYLE = "popup")
					COMMAND "systables 3"
						LET tmp =
								g2_lookup.g2_lookup(
										"systables", "tabname,created,nrows,rowsize,(nrows*rowsize)", "Name,Created,Rows,RowSize,Total",
										"tabid>99", "tabname")
					COMMAND "Stock 3"
						LET tmp =
								g2_lookup.g2_lookup(
										...
										"Code,Description,Cat,Pack,Supplier,Price,Cost,Stock,Allocated,Free", "1=1", "stock_code")
					COMMAND "Customers 3"
						LET tmp =
								g2_lookup.g2_lookup(
										"customer", "customer_code,customer_name,contact_name,total_invoices,outstanding_amount",
										"_,Customer,Contact,Invoices,Balance", "1=1", "customer_name")
				END MENU
			...
			COMMAND KEY(F19)
				LET m_answer =
						g2_core.g2_winquestion(
								"My Question", "Did you really mean to ask this?", "Yes", "Yes|No|Cancel", "question")
...
FUNCTION do1()
	...
	INPUT BY NAME norm, normbe, completr, wordw, dec, formt, check, radio, combo, c_year, dateedit, timeedit,
			datetimeedit, slider, spinedit
...
FUNCTION set_combo(l_opt)
	...
	LET rec[1].mykey  = 1
	LET rec[1].mydesc = "Astra"
	LET rec[2].mykey  = 2
	LET rec[2].mydesc = "Corsa"
	LET rec[3].mykey  = 3
	LET rec[3].mydesc = "Tigra"
	LET rec[4].mykey  = 4
	LET rec[4].mydesc = "Vectra"
...
FUNCTION textedit()
	...
	LET txt2 =
			txt2 CLIPPED,
			'<TR BGCOLOR="cyan"><TD>Line 1</TD><TD ALIGN="RIGHT">2</TD><TD ALIGN="RIGHT">15.99</TD><TD ALIGN="RIGHT">31.98</TD></TR>'
	LET txt2 =
			txt2 CLIPPED,
...
FUNCTION disp_form()
	...
	LET wordw =
			"This is some text that as been wordwrapped to fit into this text box. The field in the program is 500 characters.",
			ASCII (10), "This is on a new line using ASCII(10)"
	LET dateedit     = TODAY
	LET timeedit     = CURRENT
	LET datetimeedit = CURRENT
...
FUNCTION star(scal, lins)
	...
	LET coors =
			p1x USING "&&&", " ", p1y USING "&&&", " ", p2x USING "&&&", " ", p2y USING "&&&", " ", p3x USING "&&&", " ",
			p3y USING "&&&", " ", p4x USING "&&&", " ", p4y USING "&&&", " ", p5x USING "&&&", " ", p5y USING "&&&"
	...
	WHILE NOT int_flag
		INPUT BY NAME url ATTRIBUTE(WITHOUT DEFAULTS, UNBUFFERED)
			...
			ON ACTION close
				EXIT INPUT
			ON ACTION svgclock
				LET url = "http://borodin/gas230/components/clock/svgclock.svg"
				EXIT INPUT
			ON ACTION svg
				LET url = "http://localhost/Talon_std.svg"
				EXIT INPUT
			ON ACTION google
				LET url = "http://www.google.com/"
				EXIT INPUT
			ON ACTION fourjs
				LET url = "http://www.fourjs.com/"
				EXIT INPUT
			ON ACTION manual
				LET url = "http://www.4js.com/online_documentation/fjs-fgl-2.20.02-manual-html/"
				EXIT INPUT
