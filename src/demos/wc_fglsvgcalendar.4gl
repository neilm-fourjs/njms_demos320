...
PRIVATE CONSTANT CAL_GRID_DAYS SMALLINT = 7, CAL_GRID_WEEKS SMALLINT = 6
    ...
    FOR gcol = 1 TO CAL_GRID_DAYS
      ...
      IF cell_class IS NOT NULL THEN
        IF calendars[id].show_daynums AND calendars[id].cal_type != FGLCALENDAR_TYPE_ICON THEN
          LET t =
              fglsvgcanvas.text(tx + text_x_offset, ty + text_y_offset, DAY(day_date), dayn_class)
        ...
        IF calendars[id].show_today
                AND day_date = TODAY
                AND calendars[id].cal_type = FGLCALENDAR_TYPE_ICON
            THEN
          LET n =
              fglsvgcanvas.path(
                  -- Triangle...
...
PRIVATE FUNCTION wc_setProp(id SMALLINT, l_prop STRING, l_val STRING)
  ...
  IF m_wc_node IS NULL THEN -- if node for wc not set, then set it.
    LET m_wc_node =
        ui.Window.getCurrent().getForm().findNode("FormField", calendars[id].field).getFirstChild()
  END IF
