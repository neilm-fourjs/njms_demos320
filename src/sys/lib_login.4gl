...
--------------------------------------------------------------------------------
PRIVATE FUNCTION validate_login(l_login LIKE sys_users.email, l_pass LIKE sys_users.login_pass) RETURNS BOOLEAN
...
-- Need to actually send email!!
	...
	IF l_ret = 0 THEN -- email send okay
		UPDATE sys_users SET (salt, pass_hash, forcepwchg, pass_expire)
	...
	ELSE -- email send failed
		CALL g2_core.g2_winMessage(%"Password Reset", %"Reset Email failed to send!\nProcess aborted", "information")
	END IF
...
PRIVATE FUNCTION passchg(l_login LIKE sys_users.email) RETURNS BOOLEAN
	...
	UPDATE sys_users SET (salt, pass_hash, forcepwchg, pass_expire, hash_type)
...
-- Ignore the error if it doesn't exist
	...
	CATCH
		CALL g2_core.g2_winMessage("OpenID Error", SFMT("Failed to Parse JSON!\n%1", l_store), "exclamation")
