...
MAIN
	...
	FOREACH u_cur INTO m_user[m_user.getLength() + 1].*
		LET m_fullname[m_user.getLength()] =
				app_lib.getFullName(
						m_user[m_user.getLength()].salutation, m_user[m_user.getLength()].forenames,
	...
	DIALOG ATTRIBUTES(UNBUFFERED)
		DISPLAY ARRAY m_fullname TO u_arr.*
			BEFORE DISPLAY
				IF m_save THEN
					IF g2_core.g2_winQuestion("Confirm", "Save these changes?", "No", "Yes|No", "question") = "Yes" THEN
						CALL saveRoles_user()
					ELSE
				...
				END IF
			BEFORE ROW
				MESSAGE "DA User:", DIALOG.getCurrentRow("u_arr"), " of ", m_fullname.getLength(), " ",
		...
		DISPLAY ARRAY m_uroles TO ur_arr.*
			ON ACTION dblclick
				IF g2_core.g2_winQuestion("Confirm", "Toggle activate state for users role?", "No", "Yes|No", "question")
						= "Yes" THEN
		...
		DISPLAY ARRAY m_roles TO r_arr.*
			ON ACTION dblclick
				IF g2_core.g2_winQuestion("Confirm", "Toggle activate state for this role?", "No", "Yes|No", "question")
						= "Yes" THEN
		...
		INPUT m_user_rec.* FROM sys_users.* ATTRIBUTE(WITHOUT DEFAULTS)
			...
			BEFORE INPUT
				...
				IF m_user_rec.user_key IS NULL THEN
					...
					LET m_user_rec.salt =
							g2_secure.g2_genSalt(m_user_rec.hash_type) -- NOTE: for Genero 3.10 we don't need to store this
				...
				LET m_saveUser = FALSE
				MESSAGE "IN User:", DIALOG.getCurrentRow("u_arr"), " of ", m_fullname.getLength(), " ",
						m_user[DIALOG.getCurrentRow("u_arr")].surname
				CALL checkSave()
				...
				ELSE
					IF m_user_rec.login_pass IS NOT NULL THEN
						...
						LET m_user_rec.pass_hash =
								g2_secure.g2_genPasswordHash(m_user_rec.login_pass, m_user_rec.salt, m_user_rec.hash_type)
...
FUNCTION del_user(x)
	...
	IF m_user[x].user_key IS NULL THEN
		RETURN
	END IF
	IF g2_core.g2_winQuestion("Confirm", "Are you sure you want to delete this user?", "No", "Yes|No", "question")
			= "Yes" THEN
