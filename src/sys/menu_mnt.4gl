...
FUNCTION delete()
	...
	IF g2_core.g2_winQuestion("Confirm", "Are you sure you want to delete this menu item?", "No", "Yes|No", "question")
			= "Yes" THEN
...
FUNCTION rpt1()
	...
	IF m_wher IS NULL OR m_wher.getLength() < 1 THEN
		LET m_wher = "1=1"
	END IF
	PREPARE rpt_pre FROM "SELECT * FROM " || TABNAMEQ || " WHERE " || m_wher || " ORDER BY " || KEYFLDQ
