...
FUNCTION signin()
	...
	IF g_custcode != "Guest" THEN
		IF g2_core.g2_winQuestion(%"Confirm", %"Confirm signout", %"No", %"Yes|No", "question") = %"No" THEN
			RETURN
		END IF
	...
	LET int_flag  = FALSE
	LET l_newuser = FALSE
	INPUT BY NAME l_em, l_pwd, l_cust_name, l_cont_name, l_email, l_password1, l_password2 ATTRIBUTE(UNBUFFERED)
...
FUNCTION viewb()
	...
	LET l_co = FALSE
	INPUT ARRAY g_detailArray FROM dets.*
			ATTRIBUTE(UNBUFFERED, WITHOUT DEFAULTS, DELETE ROW = FALSE, INSERT ROW = FALSE, APPEND ROW = FALSE)
		ON ACTION delete
...
FUNCTION oe_uiUpdate()
	...
	DISPLAY BY NAME g_ordHead.total_qty, g_ordHead.total_gross, g_ordHead.total_disc, g_ordHead.total_tax,
...
-- will be del_addr if inv addr not found.
	...
	DIALOG ATTRIBUTE(UNBUFFERED)
		...
		END INPUT
		INPUT BY NAME g_ordHead.customer_code, g_ordHead.customer_name, g_ordHead.del_postcode, g_ordHead.del_address1,
				g_ordHead.del_address2, g_ordHead.del_address3, g_ordHead.del_address4, g_ordHead.del_address5,
				g_ordHead.inv_postcode, g_ordHead.inv_address1, g_ordHead.inv_address2, g_ordHead.inv_address3,
				g_ordHead.inv_address4, g_ordHead.inv_address5, m_pay.del_type, del_amt
		...
		END INPUT
		INPUT BY NAME m_pay.payment_type, m_pay.card_type, m_pay.card_no, m_pay.expires_m, m_pay.expires_y, m_pay.issue_no,
				g_ordHead.order_ref
				ATTRIBUTE(WITHOUT DEFAULTS)
			ON ACTION next
				EXIT DIALOG
	...
	SELECT * INTO l_add.* FROM addresses
			WHERE addresses.line1 = g_ordHead.del_address1 AND addresses.postal_code = g_ordHead.del_postcode
	IF STATUS != NOTFOUND THEN
		LET g_cust.del_addr = l_add.rec_key
	...
	SELECT * INTO l_add.* FROM addresses
			WHERE addresses.line1 = g_ordHead.inv_address1 AND addresses.postal_code = g_ordHead.inv_postcode
	IF STATUS != NOTFOUND THEN
	...
	END IF
	IF g_cust.inv_addr = 0 AND g_ordHead.inv_address1 IS NOT NULL AND g_ordHead.inv_postcode IS NOT NULL THEN
		LET l_add.rec_key      = 0
	...
	RUN "fglrun printInvoices.42r S 1 " || g_ordHead.order_number || " ordent-4.4rp Image bg 0 ord"
