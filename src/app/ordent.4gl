...
--	IF m_dbtyp = "sqt" THEN LET g_ordHead.order_number = NULL END IF
	...
	INPUT ARRAY g_detailArray FROM details.* ATTRIBUTE(UNBUFFERED, WITHOUT DEFAULTS) HELP 3
		...
		BEFORE FIELD quantity                                          -- Store previous qty
			SELECT free_stock INTO g_detailArray[l_row].stock FROM stock -- refresh stock value.
		...
		AFTER ROW
			...
			IF g_detailArray[l_row].stock_code IS NOT NULL THEN
				IF (g_detailArray[l_row].quantity IS NULL OR g_detailArray[l_row].quantity < 1) THEN
					...
					NEXT FIELD quantity
				END IF
				SELECT free_stock INTO m_stk.free_stock FROM stock -- refresh stock value.
						WHERE stock_code = g_detailArray[l_row].stock_code
				...
				DISPLAY BY NAME g_ordHead.items, g_ordHead.total_qty, g_ordHead.total_disc, g_ordHead.total_tax,
						g_ordHead.total_gross, g_ordHead.total_nett
		...
		BEFORE DELETE
			IF g2_core.g2_winQuestion(%"Confirm", %"Are you sure you want to remove this line?", %"No", %"Yes|No", "question")
					= "Yes" THEN
				IF NOT updateStockLevel(
						g_detailArray[l_row].stock_code, 0 - g_detailArray[l_row].quantity) THEN -- replace stock
		...
		AFTER INPUT
			IF NOT int_flag THEN
				IF g2_core.g2_winQuestion(%"Accept", %"Accept this order?", %"Yes", %"Yes|No", "question") = "No" THEN
					CONTINUE INPUT
				END IF
			ELSE
				IF g2_core.g2_winQuestion(%"Cancel", %"Cancel this order?", %"No", %"Yes|No", "question") = "No" THEN
					LET int_flag = FALSE
					CONTINUE INPUT
				END IF
			END IF
	...
	FOR l_row = 1 TO g_detailArray.getLength()
		IF g_detailArray[l_row].accepted THEN
			INSERT INTO ord_detail
					VALUES(g_ordHead.order_number, l_row, g_detailArray[l_row].stock_code, g_detailArray[l_row].pack_flag,
							g_detailArray[l_row].price, g_detailArray[l_row].quantity, g_detailArray[l_row].disc_percent,
							g_detailArray[l_row].disc_value, g_detailArray[l_row].tax_code, g_detailArray[l_row].tax_rate,
							g_detailArray[l_row].tax_value, g_detailArray[l_row].nett_value, g_detailArray[l_row].gross_value)
		END IF
	END FOR
	...
	MENU %"Order Details"
			ATTRIBUTES(STYLE = "dialog", COMMENT = SFMT(%"Order %1 Created.\nPrint Invoice?", g_ordHead.order_number),
					IMAGE = "question")
		ON ACTION continue
			EXIT MENU
		ON ACTION print
...
FUNCTION updateStockLevel(l_pcode LIKE stock.stock_code, l_qty INT) RETURNS BOOLEAN
	...
	TRY
		UPDATE stock SET (allocated_stock, free_stock) = (allocated_stock + l_qty, free_stock - l_qty)
				WHERE stock_code = l_pcode
	CATCH
		DISPLAY "Status:", STATUS, ":", SQLERRMESSAGE
		CALL g2_core.g2_errPopup(%"Unable to allocate stock!\nMaybe try again in a few minutes\nOr try a smaller quantity.")
		RETURN FALSE
	END TRY
	RETURN TRUE
END FUNCTION
...
FUNCTION enquire()
	...
	DECLARE ordCur CURSOR FOR
			SELECT stock_code, pack_flag, price, quantity, disc_percent, disc_value, tax_code, tax_rate, tax_value,
...
FUNCTION getOrder()
	...
	LET l_oh.order_number =
			g2_lookup.g2_lookup(
					"ord_head", "order_number,order_date, order_ref ,customer_name,items,total_qty,total_nett",
					"Ord No.,Date, Reference,Customer,Items,Qty,Value", "1=1", "order_number")
