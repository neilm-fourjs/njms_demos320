...
MAIN
	...
	FOR x = 1 TO base.Application.getArgumentCount()
		...
		CASE x
			WHEN 3
				LET l_ordno = base.Application.getArgument(x)
			WHEN 4
				LET l_report = base.Application.getArgument(x)
			WHEN 5
				LET l_dev = base.Application.getArgument(x)
			WHEN 6
				LET l_preview = (base.Application.getArgument(x) = "preview")
			WHEN 7
				LET l_interactive = (base.Application.getArgument(x) = "1")
		END CASE
	END FOR
...
-- Failed to setup GRE!!!
	...
	DECLARE cur2 CURSOR FOR
			SELECT stock_code, pack_flag, price, quantity, disc_percent, disc_value, tax_value, tax_code, tax_rate,
	...
	FOREACH cur INTO g_ordHead.*
		...
		FOREACH cur2
				INTO g_detailArray[g_detailArray.getLength() + 1].stock_code,
						g_detailArray[g_detailArray.getLength()].pack_flag, g_detailArray[g_detailArray.getLength()].price,
						g_detailArray[g_detailArray.getLength()].quantity, g_detailArray[g_detailArray.getLength()].disc_percent,
						g_detailArray[g_detailArray.getLength()].disc_value, g_detailArray[g_detailArray.getLength()].tax_value,
						g_detailArray[g_detailArray.getLength()].tax_code, g_detailArray[g_detailArray.getLength()].tax_rate,
						g_detailArray[g_detailArray.getLength()].nett_value, g_detailArray[g_detailArray.getLength()].gross_value
			LET l_row = g_detailArray.getLength()

			OPEN cur3 USING g_detailArray[l_row].stock_code
			FETCH cur3 INTO g_detailArray[l_row].barcode, g_detailArray[l_row].description, g_detailArray[l_row].img_url
			CLOSE cur3
				...
				FOREACH packCur USING g_detailArray[l_row].stock_code INTO l_packcode, l_pack.*
					...
					FETCH cur3
							INTO g_detailArray[g_detailArray.getLength()].barcode,
									g_detailArray[g_detailArray.getLength()].description, g_detailArray[g_detailArray.getLength()].img_url
					CLOSE cur3
				END FOREACH
			END IF
		END FOREACH
